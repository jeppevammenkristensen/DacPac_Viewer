...
- `.github/workflows/release.yml` triggers automatically when a `v*` tag is created, either by pushing the tag or by creating it from the GitHub release page (as well as manual `workflow_dispatch`). To cut a release, create a tag matching the prefix, e.g. `git tag v1.2.3 && git push origin v1.2.3` — this includes pre-release tags like `v1.2.3-beta-1`, so be deliberate about what you tag.
