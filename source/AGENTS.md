...
- `.github/workflows/release.yml` triggers automatically on any pushed tag matching `v*` (as well as manual `workflow_dispatch`). To cut a release, just push a tag matching the prefix, e.g. `git tag v1.2.3 && git push origin v1.2.3` — this includes pre-release tags like `v1.2.3-beta-1`, so be deliberate about what you tag.
